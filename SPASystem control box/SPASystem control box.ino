...
#include "RemoteControl.h" //library test
...
void setup() {  
  ...
  //object1.pruebaRemote(d1); //pruebaaaaa
  signalMemory=detectSignal(0);
}

void loop() {
 /*To turn on the Display select one of the 3 channels
    CH1 To move motor 1, sensor of conveyor
    CH2 To move motor 2, sensor of the labels X axis
    CH3 To move motor 3, sensor of the labels Y axis
  */
  if(signalMemory==true){
    
    while(IRcontrol==false){
